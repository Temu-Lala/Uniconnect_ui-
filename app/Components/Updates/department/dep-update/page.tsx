...
const DepartmentUpdate: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const fetchDepartmentProfile = async () => {
      ...
      try {
        const response = await axios.get(`http://127.0.0.1:8000/department-profiles/detail/`, {
          headers: {
        ...
        setFormData({
          ...
          college_profile_id: profileData.college,
          group: profileData.group,
        });
        fetchCampuses(profileData.university);
        fetchColleges(profileData.campus);
      } catch (error) {
  ...
  const fetchCampuses = (universityId: string) => {
    const authToken = localStorage.getItem("token");
    axios
      .get(`http://127.0.0.1:8000/university-profiles/${universityId}/campus-profiles/`, {
        headers: {
          Authorization: `Bearer ${authToken}`,
        },
      })
  ...
  const fetchColleges = (campusId: string) => {
    const authToken = localStorage.getItem("token");
    axios
      .get(`http://127.0.0.1:8000/campus-profiles/${campusId}/college-profiles/`, {
        headers: {
          Authorization: `Bearer ${authToken}`,
        },
      })
    ...
    try {
      ...
      if (authToken) {
        ...
        const response = await axios.put(
          `http://127.0.0.1:8000/department-profiles/detail/`,
          data,
          {
            headers: {
        ...
        message.success("Department profile updated successfully!");
        setFormData(initialDepartmentFormData);
      }
    } catch (error) {
      message.error("Failed to update department profile. Please try again.");
      console.error("Failed to update department profile:", error);
    }
  };
      ...
      <div className="mx-auto w-full md:w-8/12 bg-white p-8 flex justify-center border shadow-lg">
        <Form className="" layout="vertical" onFinish={handleSubmit}>
          ...
          <div className="mb-5 h-5">
            <Checkbox>
              By updating a department profile on Uni-connect you're agreeing to
              our{" "}
