...
const UpdateCampusProfile: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (authToken) {
      ...
      axios
        .get("http://127.0.0.1:8000/campus-profile/", {
          headers: {
            Authorization: `Bearer ${authToken}`,
          },
        })
        .then((response) => {
          ...
          setFormData({
            ...
            background: profile.background ? { url: profile.background } : null,
          });
    ...
    try {
      ...
      if (authToken) {
        const response = await axios.put(
          "http://127.0.0.1:8000/campus-profile/update/",
          data,
          {
            headers: {
        ...
        message.success("Campus profile updated successfully!");
        console.log("Campus profile updated:", response.data);
        setFormData(initialCampusFormData);
      }
    } catch (error) {
      message.error("Failed to update campus profile. Please try again.");
...
export default UpdateCampusProfile;
