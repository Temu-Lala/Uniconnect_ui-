...
const UpdateLecturerCVProfileForm = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        console.error('Error fetching lecturer CV:', error);
      }
    };
  ...
  const handleSubmit = async (e) => {
    ...
    if (authToken) {
      ...
      try {
        const response = await axios.put('http://127.0.0.1:8000/update-lecturer-cv/', form, {
          headers: {
            'Content-Type': 'multipart/form-data',
            'Authorization': `Bearer ${authToken}`
          },
        });
        setSuccessMessage('Lecturer CV updated successfully');
        toast.success('Lecturer CV updated successfully');
      } catch (error) {
        console.error('Error updating lecturer CV:', error);
        setError('Error updating lecturer CV. Please try again.');
        toast.error('Error updating lecturer CV. Please try again.');
      }
    }
  };
  ...
  const renderProgressBar = () => {
    return (
      <div className="flex justify-center mb-6">
        ...
        <div className={`h-2 w-1/4 ${currentStep === 4 ? 'bg-blue-500' : 'bg-gray-300'}`} />
      </div>
    );
  };
...
export default UpdateLecturerCVProfileForm;
