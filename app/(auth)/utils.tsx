...
const removeTokens = () => {
    Cookies.remove("accessToken"); // Corrected typo here
    Cookies.remove("refreshToken");
};
  
const register = (email: string, username: string, password: string, name: string, gender: string, age: number, link: string, phone: string) => {
    return new Promise((resolve, reject) => {
        api.post({ email, username, password, name, gender, age, link, phone }, "/GustUser/")
            .json((data) => {
                resolve(data);
            })
            .catch((error) => {
