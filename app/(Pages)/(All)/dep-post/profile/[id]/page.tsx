...
const DepartmentNewsFeed = ({ params }) => {
  ...
  const fetchDepartmentPost = async (postId) => {
    try {
      ...
      if (!response.ok) {
        throw new Error('Failed to fetch department post');
      }
      const data = await response.json();
      setNewsItem(formatPostItem(data));
    } catch (error) {
      setError('Failed to fetch department post. Please try again later.');
    }
  };
  ...
  const toggleComments = async (postId: number) => {
    ...
    if (updatedNewsItem.showAllComments) {
      ...
      try {
        const response = await fetch(`http://127.0.0.1:8000/comments/department/${postId}/`);
        if (!response.ok) {
  ...
  const handleSharePost = async (postId: number) => {
    try {
      ...
      const response = await fetch(
        `http://127.0.0.1:8000/share-post/department/${postId}`,
        {
          method: 'POST',
          headers: {
            'Content-Type': 'application/json',
...
export default DepartmentNewsFeed;
