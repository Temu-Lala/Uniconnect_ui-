...
const UniversityProfileDetailPage = ({ params }: { params: Params }) => {
  ...
  const fetchRatingsAndComments = async (id: string) => {
    try {
      ...
      console.error('Error fetching ratings and comments:', (error as Error).message);
    }
  };
  ...
  const checkFollowingStatus = async (id: string) => {
    try {
      const token = localStorage.getItem('token');
      const response = await axios.get(`http://127.0.0.1:8000/check-follow-university-status/university/${id}/`, {
        headers: {
      ...
      console.error('Error checking following status:', (error as Error).message);
    }
  };

  const fetchFollowersCount = async (id: string) => {
    try {
      const response = await axios.get(`http://127.0.0.1:8000/university_followers_count/${id}/`);
      setFollowersCount(response.data.followers_count);
    } catch (error) {
      console.error('Error fetching followers count:', (error as Error).message);
    }
  };
  ...
  const handleSubmit = async () => {
    try {
      ...
      console.error('Error adding rating and comment:', (error as Error).message);
    }
  };
  ...
  const handleFollow = async () => {
    try {
      ...
      const response = await axios.post(
        `http://127.0.0.1:8000/${action}-university-profile/${id}/`,
        null,
        {
          headers: {
            Authorization: `Bearer ${token}`
      ...
      console.error('Error following/unfollowing profile:', (error as Error).message);
    }
  };
  ...
  return (
    <div className="pt-24 px-4 md:px-8 lg:px-16 bg-gray-900 text-white min-h-screen space-y-8">
      {profile && (
        <div className="bg-gray-800 shadow-lg rounded-lg overflow-hidden space-y-8">
          ...
          <div className="p-4 text-center mt-12 space-y-4">
            ...
            <div className="flex justify-center space-x-4 mt-4">
              ...
              <button onClick={handleFollow} className={`btn ${isFollowing ? 'bg-red-500' : 'bg-green-500'}`}>{isFollowing ? 'Unfollow' : 'Follow'}</button>
            </div>
          </div>
          ...
          <div className="border-t border-gray-700 p-4 space-y-4">
            ...
            <div className="flex flex-wrap justify-center gap-4 text-gray-400">
              ...
              <div className="w-full md:w-auto"><FaBook className="inline-block mr-1" /><strong>Libraries:</strong> {profile.number_of_libraries}</div>
              <div className="w-full md:w-auto"><FaMicroscope className="inline-block mr-1" /><strong>Laboratories:</strong> {profile.number_of_laboratories}</div>
            </div>
            <div className={`mt-4 transition-all duration-300 ${seeMore ? 'max-h-screen' : 'max-h-0 overflow-hidden'}`}>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                ...
                <div><FaUniversity className="inline-block mr-1" /><strong>Category:</strong> {profile.category}</div>
                <div><FaRegCalendarAlt className="inline-block mr-1" /><strong>Health Condition Support:</strong> {profile.health_condition_support}</div>
                <div><FaMapMarkerAlt className="inline-block mr-1" /><strong>Status:</strong> {profile.status}</div>
          ...
          <div className="border-t border-gray-700 p-4 text-center space-y-4">
            <h2 className="text-lg font-bold mb-2">Rate and Comment</h2>
            <div className={`bg-gray-800 p-6 rounded-lg shadow-md w-full md:w-1/2 mx-auto ${getGlowEffect()} transition-shadow duration-300`}>
              <input
                type="range"
                min="0"
                max="5"
                ...
                className={`w-full mb-2 h-2 rounded-lg appearance-none cursor-pointer glow-effect ${getGlowEffect()}`}
                style={{ height: "1rem", boxShadow: `0px 0px 15px ${rating <= 2 ? 'red' : rating <= 4 ? 'yellow' : 'green'}` }}
              />
...
export default UniversityProfileDetailPage;
