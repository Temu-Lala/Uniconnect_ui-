...
import {
  ...
  FaClipboardList,
  FaHeartbeat,
  FaEllipsisV, // Import the three-dot icon
} from "react-icons/fa";
...
const CollegeDashboard = () => {
  ...
  const fetchProfile = async (userId: number, token: string) => {
    try {
      const response = await fetch(`http://127.0.0.1:8000/api/college-profiles/by-user/?user=${userId}`, {
        headers: {
          ...
          <div className="absolute top-0 right-0 p-4">
            ...
            {showOptions && (
              <div className="absolute right-0 mt-2 w-48 bg-white rounded-md shadow-lg z-10">
                <Link href="../Components/Updates/Collage/coll-update/"> {/* Update profile page */}
                  <button
                ...
                <Link href="../Components/Updates/Collage/coll-delete/"> {/* Delete profile page */}
                  <button
                    className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 w-full text-left"
                  >
                    Delete Profile
