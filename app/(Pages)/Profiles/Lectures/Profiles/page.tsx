...
const UserProfileList = () => {
  ...
  useEffect(() => {
    const fetchUserProfiles = async () => {
      try {
        ...
        const response = await fetch('http://127.0.0.1:8000/lecturer-cv/', {
          headers: {
  ...
  const handleUpdateProfile = async (profileId, updatedData) => {
    try {
      ...
      const response = await fetch(`http://127.0.0.1:8000/lecturer-cv/${profileId}/`, {
        method: 'PUT',
        headers: {
          'Authorization': `Bearer ${token}`,
    ...
    <div className="container mx-auto py-8">
      ...
      {userProfiles.map((profile) => (
        <div key={profile.id} className="bg-white shadow rounded p-4 mb-4">
          ...
          <p className="text-gray-600">Establishment Date: {profile.name}</p>
